...
import { BarChart, EmptyData, PieChart } from '..';
...
class AnswerDistributionPanel extends PureComponent {
  render() {
    const {
      chartData, entityKey, entityType, variableDefinition,
    } = this.props;
    ...
    return (
      <div className="dashboard__panel dashboard__panel--chart">
        <h4 className="dashboard__header-text">
          {entityLabel(entityKey, entityType)}
          :
          {variableDefinition.name}
          <small className="dashboard__header-subtext">
            {headerLabel(variableDefinition.type)}
            {' '}
            distribution
          </small>
        </h4>
