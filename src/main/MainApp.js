...
const createApp = () => {
  ...
  const showImportProtocolDialog = () => protocolManager.presentImportProtocolDialog(mainWindow.window)
    .then(() => mainWindow.send(FileImportUpdated))
    .catch((err) => {
      dialog.showErrorBox('Protocol Import Error', err && err.message);
    });

  const showImportSessionDialog = () => protocolManager.presentImportSessionDialog(mainWindow.window)
    .catch((err) => {
